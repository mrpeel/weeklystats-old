 ...
 */
  ...
  function renderWeekOverWeekSessions(ids, startDate, endDate) {
      ...
      .then(function(result){
        currentWeekData = result.rows.map(function(row) { return +row[2]; });
        return rateLimitDelay();
        })
      .then(function(result){
        return query({
      ...
      .then(function(result){
        previousWeekData = result.rows.map(function(row) { return +row[2]; });
        labels = result.rows.map(function(row) { return +row[0]; });
  ...
  /**
    ...
    function renderYearOverYearSessions(ids, startDate, endDate) {
      ...
      .then(function(result){
        currentYearData = result.rows.map(function(row) { return +row[2]; });
        return rateLimitDelay();
      })
      .then(function(result){
        //Previous year data
      ...
      .then(function(result){
        previousYearData = result.rows.map(function(row) { return +row[2]; });
  ...
  function renderWeekOverWeekSessionDuration(ids, startDate, endDate) {
    ...
    // ordered by day of week, then the highest to lowest number of sessions returned.
      ...
      .then(function(result){
        currentWeekData = result.rows.map(function(row) { return (+row[2]/60).toFixed(2); });
        return rateLimitDelay();
      })
      .then(function(result){
        return query({
      ...
      .then(function(result){
        previousWeekData = result.rows.map(function(row) { return (+row[2]/60).toFixed(2); });
        labels = result.rows.map(function(row) { return +row[0]; });
        labels = labels.map(function(label) {
          return moment(label, 'YYYYMMDD').format('ddd');
          });
  ...
  function renderYearOverYearSessionDuration(ids, startDate, endDate) {
      ...
      .then(function(result){
        currentYearData = result.rows.map(function(row) { return (+row[2]/60).toFixed(2); });
        return rateLimitDelay();
      })
      .then(function(result){
        //Previous year data
      ...
      .then(function(result){
        previousYearData = result.rows.map(function(row) { return (+row[2]/60).toFixed(2); });
 ...
 * When used with promise chaining, this allows control over the rate 
